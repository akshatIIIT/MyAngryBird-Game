...
public class loosing implements Screen {
    ...
    public loosing(game game,float worldWidth,float worldHeight,int currentLevel){
...
//                    ((loadGameScreen) s).LEVEL = currentLevel;
    ...
    public void EXIT(float worldWidth,float worldHeight){
        exitsprite =new Sprite(new TextureRegion(new Texture("bgicons.png"),129,173,28,26));
        ...
        exit=new Rectangle((int) exitsprite.getX(), (int) exitsprite.getY(),(int) exitsprite.getWidth(), (int) exitsprite.getHeight());
        multiplexer.addProcessor(new InputAdapter() {
            @Override
            public boolean touchDown(int screenX, int screenY, int pointer, int button) {
                Vector3 Worldcoordinates = camera.unproject(new Vector3( screenX, screenY,0));
                if (exit.contains(Worldcoordinates.x, Worldcoordinates.y)) {
//                    Screen s=new loadGameScreen(GAME,worldWidth,worldHeight);
